...
function installNginx(){
	...
	echo
	green "===============安装nginx==============="
	sudo apt install -y nginx || return 100
	yellow ">>>> nginx安装成功"
...
green "===============安装常用软件包==============="
sudo apt -y update
sudo apt -y install unzip zip wget curl vim socat ntp ntpdate gcc git xz-utils || exit 100
...
yellow ">>>>>>>> 申请证书"
sudo ~/.acme.sh/acme.sh  --issue  -d $domain  \
	--webroot /usr/share/nginx/html/  \
	-k ec-256  \
	--force  --debug || exit 301
sudo installCert $domain || exit 302
...
# 安装trojan
sudo installTrojan $domain || exit 200
